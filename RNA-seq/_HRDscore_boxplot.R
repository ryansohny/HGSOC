...
p2 <- ggplot(dat, aes(x=Cluster, y=TelomericAI, fill=Cluster)) + geom_violin(trim=TRUE) + geom_dotplot(binaxis='y', stackdir='center', dotsize=0.7) + scale_fill_manual(values=c('#000066','#990000'))  + theme_classic()
...
p4 <- ggplot(dat, aes(x=Cluster, y=HRDsum, fill=Cluster)) + geom_violin(trim=TRUE) + geom_dotplot(binaxis='y', stackdir='center', dotsize=0.7) + scale_fill_manual(values=c('#000066','#990000'))  + theme_classic()
...
dat %>% wilcox_test(TelomericAI ~ Cluster, exact=TRUE, detailed=TRUE, p.adjust.method="bonferroni")
dat %>% wilcox_test(LST ~ Cluster, exact=TRUE, detailed=TRUE, p.adjust.method="bonferroni")
dat %>% wilcox_test(HRDsum ~ Cluster, exact=TRUE, detailed=TRUE, p.adjust.method="bonferroni")
